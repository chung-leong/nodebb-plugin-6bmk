...
Helpers.createFlyer = async (options) => {
...
Helpers.useHaiku = async (hid, uid) => {
  ...
  db.sortedSetRemove(HAIKU_SET_KEY, key);
  db.sortedSetAdd(USED_HAIKU_SET_KEY, key);
};
...
Helpers.findUsedHaiku = async (text) => {
  const haiku = await loadSet(USED_HAIKU_SET_KEY);
  const hash = getHash(text);
  return haiku.find(h => h.hash === hash);
};
...
Helpers.getFlyerStream = async (fid) => {
  ...
  const stream = await createFlyer({ haiku, address, ...flyer.options });
