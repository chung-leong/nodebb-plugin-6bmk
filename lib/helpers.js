...
async function loadSet(set, start = 0, end = -1) {
  const keys = await db.getSortedSetRange(set, start, end);
...
async function findHaiku(text, unused = true) {
  ...
  const haiku = await loadSet(unused ? HAIKU_SET_KEY : USED_HAIKU_SET_KEY, score, score);
